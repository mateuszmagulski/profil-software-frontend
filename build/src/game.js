...
class Game {
    ...
    playerEndRound(result) {
        ...
        if (this.active_player.id === this.players.length - 1 &&
            !this.winner.length) {
            message = `${this.players[this.players.length - 1].name} wygrał, inni uzyskali więcej niż 21 punktów`;
            this.active = false;
            this.render(message);
            return;
        }
